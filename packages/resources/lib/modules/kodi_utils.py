...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.fenlight.am').getAddonInfo(info)
...
def build_url(url_params):
	return 'plugin://plugin.video.fenlight.am/?%s' % urlencode(url_params)
...
def addon(addon_id='plugin.video.fenlight.am'):
...
def run_addon(addon='plugin.video.fenlight.am', block=False):
...
def disable_enable_addon(addon_name='plugin.video.fenlight.am'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.fenlight.am'):
	import time
