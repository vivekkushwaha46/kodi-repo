...
def rollback_check():
	...
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results if 'plugin.video.fenlight.am' in i['name'] \
...
def update_addon(new_version, action, show_after_action=True):
	...
	zip_name = 'plugin.video.fenlight.am-%s.zip' % new_version
	...
	shutil.rmtree(path.join(kodi_utils.translate_path('special://home/addons/'), 'plugin.video.fenlight.am'))
	success = unzip(zip_location, kodi_utils.translate_path('special://home/addons/'), kodi_utils.translate_path('special://home/addons/plugin.video.fenlight.am/'))
	...
	if action == 5:
		...
		if show_after_action:
			if kodi_utils.confirm_dialog(heading='Fen Light Updater', text='[CR]Success.[CR]Fen Light updated to version [B]%s[/B]' % new_version,
				...
				kodi_utils.show_text('Changelog', file=kodi_utils.translate_path('special://home/addons/plugin.video.fenlight.am/resources/text/changelog.txt'), font_size='large')
		else:
