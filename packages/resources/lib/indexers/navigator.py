...
class Navigator:
	...
	def changelog_utils(self):
		...
		fenlight_clogpath = k.translate_path('special://home/addons/plugin.video.fenlight.am/resources/text/changelog.txt')
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.fenlight.am' in iconImage: original_image = True
	...
	def tips(self):
		tips_location = 'special://home/addons/plugin.video.fenlight.am/resources/text/tips'
